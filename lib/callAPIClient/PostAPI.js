...
export const createPost = async (form) => {
    try {
        ...
        var body = {
            ...
            IsCommunityPost: form.communityId ? true : false
        }
...
export const getUserPost = async (userId) => {
    try {
        const response = await GetRequest('/post/userpost/' + userId);
        if (response) {
            return response;
        }
        return null;
...
export const getNewPostCommunity = async (communityId, page = 1, limit = 20) => {
    try {
        const response = await GetRequest('/post/getNewPostCommunity/' + communityId + '/' + page + '/' + limit);
        if (response) {
            return response;
        }
        return null;
